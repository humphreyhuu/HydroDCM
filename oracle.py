...
class OracleDataset(Dataset):
    """Dataset for oracle training (only X and y, no spatial info)"""
...
def train_oracle(model, test_loader, device, 
                 num_epochs=100, lr=1e-3, save_dir="./logs/HydroDCM"):
    """Training loop for oracle ED-LSTM (train directly on test set)"""
    ...
    best_model_path = os.path.join(save_dir, 'oracle_best_model.pth')
    
    print(f"Training Oracle ED-LSTM for {num_epochs} epochs...")
    print("Note: Training directly on TARGET test set (theoretical upper bound)")
    
    for epoch in range(num_epochs):
        # Training phase (using test set as training data)
        model.train()
        train_loss = 0
        
        train_pbar = tqdm(test_loader, desc=f"Epoch {epoch+1}/{num_epochs} [Train on Test]", leave=False)
        ...
        avg_train_loss = train_loss / len(test_loader.dataset)
        ...
        if avg_train_loss < best_train_loss:
            ...
            print(f"  → New best model saved (train_loss: {avg_train_loss:.4f})")
...
def main():
    ...
    # Use only TARGET domain data for oracle training
    print("\nUsing TARGET domain data for oracle training (theoretical upper bound)")
    print("Oracle setup: Train directly on test set (no train/val split needed)")
    
    # Create dataset using the full target test set for training
    ...
    print(f"Oracle training samples: {len(test_dataset)} (target domains)")
    print(f"Target reservoirs: {target_reservoirs}")
    print("Note: Training and testing on the same data (theoretical upper bound)")
    ...
    history, best_model_path = train_oracle(
        model=model,
    ...
    print("\nEvaluating oracle on target domains...")
    ...
    results_file = os.path.join(model_save_dir, f'oracle_results_{timestamp}.json')
    
    results = {
        'model_type': 'oracle_ed_lstm',
        'description': 'Upper bound: ED-LSTM trained on target domains, tested on target domains (in-domain)',
        'hyperparameters': {
            'input_dim': input_dim,
            'hidden_dim': hidden_dim,
            'output_dim': output_dim,
    ...
    print("This represents the UPPER BOUND performance (in-domain training)")
